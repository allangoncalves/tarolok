...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv tarolok` or `python -m venv tarolok-venv`.
...
- [ ] Make sure the virtualenv is activated `workon tarolok` or `source tarolok-venv/bin/activate`.
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `tarolok/backend/tarolok/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `tarolok/backend/tarolok/settings/production.py`
...
- Create a copy of ``tarolok/settings/local.py.example``:  
  `cp tarolok/settings/local.py.example tarolok/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/tarolok/settings/base.py``.
- Adjust ``backend/tarolok/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
